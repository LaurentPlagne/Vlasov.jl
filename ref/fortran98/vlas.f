...
c---------------------declaration---------------------------------
      ...
      real*8 delvm,rcmax
      real*8 gridrad(0:NBGEM)
      real*8 potrad(0:NBGEM)
      integer nbgridrad
      real*8 gridene(0:NBGEM)
      real*8 densene(0:NBGEM)
      integer nbgridene
      character*3 chnomb
      ...
      real*8 potexa(0:nbtdmax),gridexa(0:nbtdmax)
      integer nbgridexa
      character*23 titreq
      nbdt=1000
      nbcap=0
...
c           ------- calcul de qpold -------------------
     ...
     +              nbelec,volm1big,tabout,nbout,nbcap)
      ...
      call sortierho(nx,ny,nz,ntx,nty,ntz,'rhd',
...
c     +     inttab1,nbdt,pasgrid)
      call sortie(gx,gy,gz,nx,ny,nz,csol,'tot')
      call sortie(gxbig,gybig,gzbig,nx,ny,nz,csolbig,'tob')
      call sortietest(gx,gy,gz,gxbig,gybig,gzbig,
     +     nx,ny,nz,csol,csolbig)
...
c     ------------------------------------------  
     ...
     +     enele,einout,elin,nbcap,liste2,rcmax)
         end if
         call pspech(ntx,nty,ntz,gtxbig,gtybig,gtzbig,
     +     rhobig,csolbig,sxm1big,sym1big,szm1big,nbion)
         call pspech(ntx,nty,ntz,gtx,gty,gtz,
     +        rho,csol,sxm1,sym1,szm1,nbion)
c         if ((i.gt.nbatt).and.(i.le.(nbatt+nbphot))) then
c            call makecha(chnomb,i-nbatt)
c            titi='t'//chnomb
c            tito='r'//chnomb
c            print*,'titi=',titi
c            if (mod(i-1,5).eq.0) then  
c              call sortie(gxbig,gybig,gzbig,nx,ny,nz,csolbig,titi)    
c             end if
c            call sortierho(nx,ny,nz,ntx,nty,ntz,tito,
...
c
     ...
     +        nbout,tabout,lcine,nbcap,rcmax)
c         call denseta(npart,nbelec,qp,fp,nx,ny,nz,
c     +     csol,gx,gy,gz,inttab1,nbdt,pasgrid)
c         epoti=epotip1
c         if ((mod(i,20).eq.0).or.(i.eq.nbt)) then
         if (mod(i-1,10).eq.0) then    
c         call pspech2(ntx,nty,ntz,gtxbig,gtybig,gtzbig,
c     +     rhobig,csolbig,sxm1big,sym1big,szm1big,nbion)
c         call pspech2(ntx,nty,ntz,gtx,gty,gtz,
c     +        rho,csol,sxm1,sym1,szm1,nbion)
c         call sortietest2(gx,gy,gz,gxbig,gybig,gzbig,
c     +        nx,ny,nz,csol,csolbig)
c         call enertot2g(npart,nbion,nbelec,qp,nx,ny,nz,
c     +     csolbig,gxbig,gybig,gzbig,nbout,
c     +     csol,gx,gy,gz,inttab1,nbdt,pasgrid,
c     +     enele,einout,elin,enejel,ekin,
c     +     ekinout,enetot,i/10+1,lcine,first2,
c     +     nbcap,liste2,titreq,rcmax)
c                     enerin
         call makerhogi(rho,npart,qpold,nx,ny,nz,gx,gy,gz,
     +        nbelec,tabouti,nbouti,nbdt,pasgrid,gausstab,nbcap,
     +     cofrho,sxm1,sym1,szm1,ntx,nty,ntz,psx,psy,psz,rcmax)
         call makerhoi(rhobig,npart,qpold,ntx,nty,ntz,
     +              gtxbig,gtybig,gtzbig,
     +              nbelec,volm1big,tabout,nbout,nbcap,rcmax)
         call makerh2(rhobig,dxbig,dybig,dzbig,nsx,nsy,nsz,
     +     gtxbig,gtybig,gtzbig,phibig,rhslbig,
     +     sxm1big,sym1big,szm1big,
     +     psxbig,psybig,pszbig,psxxbig,
     ...
     +     rhobig,csolbig,sxm1big,sym1big,szm1big,nbion)
         call pspech(ntx,nty,ntz,gtx,gty,gtz,
     +        rho,csol,sxm1,sym1,szm1,nbion)
         call enertot2gix(npart,nbion,nbelec,qpold,nx,ny,nz,
     +     csolbig,gxbig,gybig,gzbig,nbout,
     +     csol,gx,gy,gz,inttab1,nbdt,pasgrid,
     +     enele,einout,elin,enejel,ekin,
     +     ekinout,enetot,i/10+1,lcine,first2,
     +     nbcap,liste2,titreq,rcmax,
     +     fp,dltt,potexa,gridexa,nbgridexa)
         print*,'enertot2gi ok'
         call mkpotradx(nbion,nbelec,nx,ny,nz,npart,
     +        csol,gx,gy,gz,inttab1,nbdt,pasgrid,
     +        gridrad,potrad,nbgridrad,potexa,gridexa,nbgridexa)
         print*,'mkpotradx ok'
         call mkdensene(gridrad,potrad,nbgridrad,
     +        gridene,densene,nbgridene)
         print*,'mkdensene ok'
         call distene(nbion,nbelec,fp,npart,
     +        gridene,densene,nbgridene)      
         print*,'distene ok'
c         call distene(nbion,nbelec,fp,npart)
c                  fin enerin
         call angular(npart,qp,gx,nx,i/10+1)
         call multrcmax(npart,qp,gx,nx,i/10+1,nbelec)
         call echanti2(npart,qp,100,gtech,i/10,nbcap,nbelec)
         end if 
         call trace(qp,npart,first,i+1,nbion,nbcap)
c         call capture(npart,qp,pospro,nbelec,nbcap)
c         if (capturee) then
c         else
c            if (pospro(1).gt.(1000.d0)) then
c               call docapture(npart,qp,qpold,
c     +           fp,pospro,nbcap,chapro,nbelec,cutoff,einterne)
c               capturee=.true.
c               print*,'capture effectuee !!!'
c            end if
c         end if
      end do 
...
c      call initialize(qp,qpold,rhorp,rmax,pmax,npart,nr,np,nbelec)
      call initialise4(qp,qpold,npart,nbelec)
      end
...
c----------------------------------------------------------------------------
     ...
     +     n1big,n2big,delvm,rcmax)
      ...
      integer n1big,n2big
      real*8 xclu,xboite,nbion,dltt,chapro,rcmax
      real*8 mpro,epro,impara,cutoff,xinit
     ...
     +     sxm1big,sym1big,szm1big,gxbig,gybig,gzbig,rhobig)
      ...
      do j=0,120
         ...
         do l=0,120
            ...
            y=ymin+l*h2
            big=0
            if (x.gt.gx(nx-2)) big=big+1
            if (y.gt.gy(ny-2)) big=big+1
            if (x.lt.gx(2)) big=big+1
            if (y.lt.gy(2)) big=big+1
            if (big.eq.0) then
...
C-----------------------------------------------------
      subroutine sortietest2(gx,gy,gz,gx2,gy2,gz2,nx,ny,nz,csol,csol2)
      include 'ceq3d.f'
      real*8 gx(0:NHFX)
      real*8 gy(0:NHFY)
      real*8 gz(0:NHFZ)
      ...
      open (2,file='test2.dat',status='unknown')
c      print*,'plot x=0 => entrez 1'
c      print*,'plot y=0 => entrez 2'
c      print*,'plot z=0 => entrez 3'
c      read*, rep
     ...
     +           dumi,dumj,dumr)
      ...
      close(2)
      open (2,file='test3.dat',status='unknown')
c      print*,'plot x=0 => entrez 1'
c      print*,'plot y=0 => entrez 2'
c      print*,'plot z=0 => entrez 3'
c      read*, rep
      do j=0,100
         h=(gx2(nx)-gx2(0))/100.d0-1.d-10
         x=gx2(0)+j*h
         do l=0,100
            h2=(gz2(nz)-gz2(0))/100.d0-1.d-10
            z=gz2(0)+l*h2
            rr=dsqrt(x**2+z**2)
            pot=potjel(rr,196.d0)
            call potentiel(x,nx,gx2,z,ny,gy2,0.d0,nz,gz,csol2,pot2,
     +           dumi,dumj,dumr)
            write(2,'(4e14.6)') x,z,pot2,pot
         end do
      end do
      end
c--------------------------------------------------------------------
...
c-----------------------------------------------------------
     ...
     +     enele,einout,elin,nbcap,liste2,rcmax)
      ...
      real*8 pot,dumr,pote,ract,rcmax
      integer dumi,dumj
      integer i,j
      coef=nbelec/dfloat(npart)
...
c-----------------------------------------------------------
     ...
     +     enetot,nbtour,lcine,first,nbcap,liste2,titreq,rcmax)
      include 'ceq3d.f'
      real*8 gx(0:NHFX)
      real*8 gy(0:NHFY)
      real*8 gz(0:NHFZ)
...
c----------------------------------------------------------------------
      ...
      close(1)
      print*,'hm1(1),hm1(2),hm1(nbgrid)',hm1(1),hm1(2),hm1(nbgrid)
     ...
     +	          (x(1)-xj1)/(xj2-xj1)
         ...
         stheta= dsqrt(1.d0-(2.d0*x(3)-1.d0)**2)
         rt(1,i)= r*dcos(2.d0*PI*x(2))*stheta
         rt(2,i)= r*dsin(2.d0*PI*x(2))*stheta
         rt(3,i)= r*(2.d0*x(3)-1.d0)
...
c-----------------------------------------------------------------
     ...
     +                nbout,tabout,lcine,nbcap,rcmax)
      ...
      do i=1,npart-nbcap
         ...
         ekpart=coef*(px**2.d0+py**2.d0+pz**2.d0)
         ekin=ekin+ekpart
         fp(1,i)=ekpart
...
c------------------------------------------------------------------------
      ...
      include 'ceq3d.f'      
      real*8 qp(3,npartmax),tabang(0:NBGEM)
...
c------------------------------------------------------------------------
      subroutine multrcmax(npart,qp,gx,nx,nbtour,nbelec)
      include 'ceq3d.f'      
      real*8 qp(3,npartmax)
      real*8 gx(0:NHFX),nbelec
      integer nx,nbtour,npart
c
      real*8 rcmax(0:10),cin(0:10),deltarc,dum(0:10)
      real*8 ract
      integer i,j,nbin(0:10)
      rcmax(0)=50.d0
      rcmax(10)=100.d0
      deltarc=(rcmax(10)-rcmax(0))/10
      do i=1,9
         rcmax(i)=rcmax(0)+dfloat(i)*deltarc
      end do
      do i=0,10
         nbin(i)=0
      end do
      ...
      do i=0,10
         cin(i)=dfloat(nbin(i))*nbelec/dfloat(npart)
         print*,i,rcmax(i),cin(i)
      end do
      if (nbtour.eq.1) then
         open (1,file='rcm.dat',status='unknown')         
c          write (1,'(11e14.6)') cin(0),cin(1),cin(2),cin(3),cin(4),
c     +           cin(5),cin(6),cin(7),cin(8),cin(9),cin(10)
            write (1,'(11e14.6)') (cin(i),i=0,10)
         close(1)
      else
         open (1,file='rcm.dat',status='old')
         do j=2,nbtour
c            read (1,'(11e14)') cin(0),cin(1),cin(2),cin(3),cin(4),
c     +           cin(5),cin(6),cin(7),cin(8),cin(9),cin(10)
            read (1,'(11e14.6)') (dum(i),i=0,10)
         end do
c            write (1,'(11e14)') cin(0),cin(1),cin(2),cin(3),cin(4),
c     +           cin(5),cin(6),cin(7),cin(8),cin(9),cin(10)
            write (1,'(11e14.6)') (cin(i),i=0,10)
         close(1)
      end if
...
c-------------------- makerhogi -----------------------------------
      subroutine makerhogi(rho,npart,qp,nx,ny,nz,gx,gy,gz,
     +     nbelec,tabout,nbout,nbdt,pasgrid,gausstab,nbcap,
     +     cofrho,sxm1,sym1,szm1,ntx,nty,ntz,psx,psy,psz,rcmax)
      include 'ceq3d.f'
      real*8 psx(0:NTXT),psy(0:NTYT),psz(0:NTZT)
      ...
      real*8 cofrho(0:NTXT,0:NTYT,0:NTZT)
      real*8 charge,qtot,coef
      real*8 qp(3,npartmax),lxs2,uslx,rnbdt,rcmax
      integer tabout(npartmax),nbdt
      integer i,j,k,l,nbout,ntx,nty,ntz
      logical inx,iny,inz
      integer nx,ny,nz
      ...
      do l=1,npart-nbcap
         if ((qp(1,l)**2+qp(2,l)**2+qp(3,l)**2).lt.rcmax**2) then
            call findig(gx,nx,qp(1,l),i,inx)
            call findig(gy,ny,qp(2,l),j,iny)
            call findig(gz,nz,qp(3,l),k,inz)
            if (inx.and.iny.and.inz) then
     ...
     +              qp(1,l),qp(2,l),qp(3,l),rho)
      ...
      print*,'nb de pseudo part > rcmax :',nbout
      end  
c------subroutinemakerhoi(nbion,ntx,nty,ntz,gtx,gty,gtz,rhojel,volm1)--
      subroutine makerhoi(rho,npart,qp,ntx,nty,ntz,gtx,gty,gtz,
     +     nbelec,volm1,tabout,nbout,nbcap,rcmax)
      include 'ceq3d.f'
      real*8 gtx(0:NTXT)
      real*8 gty(0:NTYT)
      real*8 gtz(0:NTZT)
...
c------------------------------------------------------------------
      subroutine pspech3(ntx,nty,ntz,gtx,gty,gtz,
     +     rho,csol,sxm1,sym1,szm1,nbion)
      include 'ceq3d.f'              
      real*8 gtx(0:NTXT)
      real*8 gty(0:NTYT)
...
c     ech(i,j,k)=potjel(rr,nbion)
      ...
      do k=0,ntz
         do j=0,nty
            do i=0,ntx
               csol(i,j,k)=0.5d0*csol(i,j,k)+echsol(i,j,k)
            end do
         end do
      end do
      end            
c-----------------------------------------------------------
      subroutine enertot2gi(npart,nbion,nbelec,qp,nx,ny,nz,
     +     csolbig,gxbig,gybig,gzbig,nboutbig,
     +     csol,gx,gy,gz,inttab1,nbdt,pasgrid,
     +     enele,einout,elin,enejel,ekin,ekinout,
     +     enetot,nbtour,lcine,first,nbcap,liste2,titreq,rcmax,
     +     fp,dltt)
      ...
      real*8 lcine(3)
      real*8 pot,pote,dumr,ract
      real*8 fp(3,npartmax),dltt,vx,vy,vz
      real*8 ekpart,potepart,unsdltt,masse,etotpart
      real*8 maxekpart
      integer nonlie
      character*23 titreq
      integer dumi,dumj,nbtour
     ...
     +           inttab1,nbdt,pasgrid)
            ...
            potepart=coef*pote
            fp(2,i)=potepart
            fp(3,i)=potepart+ekpart
            if (ract.lt.rcmax) then
               potin=potin+coef*pote
            end if
            etotpart=ekpart+potepart
            if (i.lt.10) then
               print*,'i,ekpart,potepart,etotpart'
               print*,i,ekpart,potepart,etotpart,fp(3,i)
            end if
            if ((potepart+ekpart).gt.0.) then
               nonlie=nonlie+1
            end if
         else
            nonlie=nonlie+1
     ...
     +            ,csolbig,pot,dumi,dumj,dumr)
      ...
      rayon=(nbion**(1.d0/3.d0))*rs
      eion=3.d0*(nbion**2.d0)/(rayon*5.d0)
      enetot=eion+ekin+potel
      etotin=potin+eion+ekin-ekinout
      if (first) then
         open (1,file='eneri.dat',status='unknown')
         open (2,file='moment.dat',status='unknown')
         write(1,*) 'enetot,ekin,enele,enejel,einout,ekinout,etin'
         write(2,*) 'moment cinetique (lx,ly,lz)'
         write(1,'(7e15.7)') 
     +        enetot,ekin,enele,enejel,einout,ekinout,etotin
         ...
         open (1,file='eneri.dat',status='old')
         open (2,file='moment.dat',status='old')
         read(1,*)
         read(2,*)
         do j=1,nbtour-1
...
c---------------------------------------------------------------------
      subroutine denseta(npart,nbelec,qp,fp,nx,ny,nz,
     +     csol,gx,gy,gz,inttab1,nbdt,pasgrid)
      include 'ceq3d.f'
      real*8 gx(0:NHFX)
      real*8 gy(0:NHFY)
...
c
      real*8 xmin,xmax,ymin,ymax,zmin,zmax
      real*8 x,y,z
      real*8 p2,ekin,coef,coef2,etot,pote
      real*8 ekmin,ekmax,potemin,potemax,etotmin,etotmax
      real*8 etotdeb,etotfin,potedeb,potefin,ekindeb,ekinfin
      integer tabpote(0:NBGEM),tabekin(0:NBGEM),tabetot(0:NBGEM)
      real*8 gridpote(0:NBGEM),gridekin(0:NBGEM),gridetot(0:NBGEM)
      integer i,nbs,j
c
      nbs=100
      etotdeb=-5.d-4
      ...
      potefin=5.d-4
      do i=0,nbs
         gridpote(i)=potedeb+dfloat(i)*(potefin-potedeb)/dfloat(nbs)
         gridetot(i)=etotdeb+dfloat(i)*(etotfin-etotdeb)/dfloat(nbs)
         gridekin(i)=ekindeb+dfloat(i)*(ekinfin-ekindeb)/dfloat(nbs)
      end do
      xmin=gx(2)
      xmax=gx(nx-2)
      ymin=gy(2)
     ...
     +       (z.gt.zmin).and.(z.lt.zmax)) then
            call potensg(x,nx,gx,y,ny,gy,z,nz,gz,csol,pote,
     +           inttab1,nbdt,pasgrid)         
            p2=fp(1,i)**2+fp(2,i)**2+fp(3,i)**2
            ekin=coef2*p2
            pote=pote*coef
            etot=ekin+pote
c
            if (gridekin(0).gt.ekin) then
               print*,'probleme, gridekin(0).gt.ekin',ekin
               stop
            else
               j=0
               do while ((gridekin(j).lt.ekin).and.(j.lt.nbs))
                  j=j+1
               end do
               tabekin(j)=tabekin(j)+1
            end if
c
            if (gridetot(0).gt.etot) then
               print*,'probleme, gridetot(0).gt.etot',etot
               stop
            else
               j=0
               do while ((gridetot(j).lt.etot).and.(j.lt.nbs))
                  j=j+1
               end do
               tabetot(j)=tabetot(j)+1
            end if
c
            if (gridpote(0).gt.pote) then
               print*,'probleme, gridpote(0).gt.epote',pote
               stop
            else
               j=0
               do while ((gridpote(j).lt.pote).and.(j.lt.nbs))
                  j=j+1
               end do
               tabpote(j)=tabpote(j)+1
            end if
            if (potemin.gt.pote) potemin=pote
            if (potemax.lt.pote) potemax=pote
            if (ekmin.gt.ekin) ekmin=ekin
            if (ekmax.lt.ekin) ekmax=ekin
            if (etotmin.gt.etot) etotmin=etot
            if (etotmax.lt.etot) etotmax=etot
         end if
      end do
      open (1,file='distekin.dat',status='unknown')
      do i=0,nbs
         write (1,'(1e14.6,I10)') gridekin(i),tabekin(i)
      end do
      close (1)
      open (1,file='distetot.dat',status='unknown')
      do i=0,nbs
         write (1,'(1e14.6,I10)') gridetot(i),tabetot(i)
      end do
      close (1)
      open (1,file='distpote.dat',status='unknown')
      do i=0,nbs
         write (1,'(1e14.6,I10)') gridpote(i),tabpote(i)
      end do
      close (1)
      print*,'potemin=',potemin
      print*,'potemax=',potemax
      print*,'ekmin=',ekmin
      print*,'ekmax=',ekmax
      print*,'etotmin=',etotmin
      print*,'etotmax=',etotmax
      end
c---------------------------------------------------------------------
      subroutine distene(nbion,nbelec,fp,npart,
     +               gridene,densene,nbgridene)      
      include 'ceq3d.f'
      real*8 fp(3,npartmax)      
      real*8 nbion,nbelec
c      
      integer i,npart,j
      integer nbgridene
      real*8 ekpart,potpart,etot
      real*8 emin,emax
      real*8 hgride,energie
      real*8 gridene(0:NBGEM)
      real*8 densene(0:NBGEM)
      real*8 coef
      integer tabene(0:NBGEM)
      real*8 tabener(0:NBGEM)
      integer nbgridrad
      coef=float(npart)/nbelec
      do i=1,npart
         j=0
         do while ((coef*fp(3,i)).ge.gridene(j))
            j=j+1
         end do
         j=j-1
         tabene(j)=tabene(j)+1
      end do
      tabener=dfloat(tabene)/coef
      open (1,file='distene.dat',status='unknown')
      do i=1,nbgridene
         if (densene(i).eq.0.d0) then
            write (1,'(2e14.6,1I7)') gridene(i)
     +           ,0.0,tabene(i)
         else
            write (1,'(2e14.6,1I7)') gridene(i)
     +           ,tabener(i)/densene(i),tabene(i)            
         end if
      end do
      close (1)
      end
c-----------------------------------------------------------
      subroutine mkpotrad(nbion,nbelec,nx,ny,nz,npart,
     +     csol,gx,gy,gz,inttab1,nbdt,pasgrid,
     +     gridrad,potrad,nbgridrad)
      include 'ceq3d.f'
      real*8 gx(0:NHFX)
      real*8 gy(0:NHFY)
      real*8 gz(0:NHFZ)
      ...
      integer nbgridrad
c
      real*8 coef,potel,pote,hgrid
      integer i,j,nbang
      real*8 x,y,z,ang,hang,r,cofx,cofy,rmax
c
      coef=nbelec/dfloat(npart)
c     
      rmax=50.0
      nbgridrad=100
      hgrid=rmax/float(nbgridrad)
      do i=0,nbgridrad
         gridrad(i)=float(i)*hgrid
      end do
c
      nbang=50
      hang=2*pi/float(nbang)
      z=0.0
      potrad=0.0
      do j=0,nbang
         ang=float(j)*hang
     ...
     +           inttab1,nbdt,pasgrid)
            potel=coef*pote
            potrad(i)=potrad(i)+potel
         end do
      end do
      potrad=potrad/float(nbang)
      open (1,file='potrad.dat',status='unknown')
      do i=0,nbgridrad
         write (1,'(3e14.6)') gridrad(i),potrad(i)
     +        ,potrad(i)*float(npart)/nbelec
      end do
      close (1)
      end      
c------------------------------------------------------------------------
      subroutine mkdensene(gridrad,potrad,nbgridrad,
     +     gridene,densene,nbgridene)      
      include 'ceq3d.f'
      real*8 gridrad(0:NBGEM)
      real*8 potrad(0:NBGEM)
...
c
      integer i,nbinter,j
      real*8 emin,emax,hene,ecour
      real*8 rinf,rsup,finf,fsup,potesup,poteinf
      real*8 coefinf,coefsup,coef2,coef
c
      coef=dsqrt(2.d0)/(PI**2.d0)
      emin=1000.0
      emax=-1000.0
      do i=0,nbgridrad
         if (emin.gt.potrad(i)) emin=potrad(i)
         if (emax.lt.potrad(i)) emax=potrad(i)
      end do
      emin=-0.45
      emax=0.05
      nbgridene=100
      hene=(emax-emin)/float(nbgridene)
      do i=0,nbgridene
         gridene(i)=emin+float(i)*hene
      end do
      nbinter=100
      densene=0.0
      do j=0,nbgridrad-1
         ...
         do i=0,nbgridene
            ecour=gridene(i)
            if (ecour.lt.poteinf) then
               finf=0.d0
            else
               finf=coefinf*dsqrt(ecour-poteinf)
            end if
            if (ecour.lt.potesup) then
               fsup=0.d0
            else
               fsup=coefsup*dsqrt(ecour-potesup)
            end if
            densene(i)=densene(i)+coef2*(fsup+finf)
         end do
      end do
      densene=densene*coef*hene
      open (1,file='densene.dat',status='unknown')
      do i=0,nbgridene
         write (1,'(2e14.6)') gridene(i),densene(i)
      end do
      close (1)
      end
c----------------------------------------------------------------------
      subroutine testinit(rt,pt,npart,nbelec)
      include 'ceq3d.f'
      real*8 rt(3,npartmax)
      real*8 pt(3,npartmax)
      real*8 x(6),nbelec
      real*4 ran2
      real*8 hm1(nbtdmax),rel(nbtdmax)
      integer i,npart,nbgrid,nbgrid2,rmax,nbtini
      real*8 r,p,rhor,rk1,rk2,xj1,xj2,coef,pf
      integer idum,nbi,j,k
      real*8 stheta,ee
      open (1,file='hm1.dat',status='old')
      ...
      idum=-1
      coef=(3.d0*(PI**2))**(1.d0/3.d0)
      open (1,file='tini.dat',status='unknown')
      nbtini=9000
      do i=1,nbtini
         x(1)=float(i-1)/float(nbtini-1)
	 j=int(x(1)*dfloat(nbgrid-1)-1.d-10)+1
         xj1=dfloat(j-1)/dfloat(nbgrid-1)
         xj2=dfloat(j)/dfloat(nbgrid-1)
         r=hm1(j)+(hm1(j+1)-hm1(j))*
     +	          (x(1)-xj1)/(xj2-xj1)
         ...
         write (1,'(2e15.6)') x(1),r
      end do
      close(1)
      stop
      end           
c----------------------------------------------------------------------
      subroutine initialise4(qp,pp,npart,nbelec)
c
      include 'ceq3d.f'
      real*8 qp(3,npartmax)
      real*8 pp(3,npartmax)
      real*8 x(6),nbelec
      real*4 ran2
      real*8 tabpotr(0:nbtdmax),gridr(0:nbtdmax)
      real*8 rmax,pmax,Ef,r,p,Ek,pot,Et,stheta
      real*8 coef,coef2,ee,dumx
      integer i,npart,nbgrid,idum
      integer nbi,j,count
      logical rejet
c
      count=0
      coef=(3.d0*(PI**2))**(1.d0/3.d0)
      ee = nbelec/dfloat(npart)
      open (1,file='pot.dat',status='old')
      read (1,*) nbgrid
      read (1,*) rmax,pmax,Ef
      do i=0,nbgrid
           read (1,'(3e14.6)') gridr(i),dumx,tabpotr(i)
      end do	
c
      print*,'rmax,pmax,ef',rmax,pmax,ef
      print*,'gridr(0),tabpotr(0)',gridr(0),tabpotr(0)
      print*,'gridr(nbgrid),tabpotr(nbgrid)'
     +     ,gridr(nbgrid),tabpotr(nbgrid)
c
      coef2=dfloat(nbgrid)/rmax
      close(1)
      idum=-1
      do i=1,npart 
c
...
c     
            Ek=0.5d0*(p**2)
            j=int(coef2*r-1.d-10)
            pot=tabpotr(j)+(tabpotr(j+1)-tabpotr(j))*
     +           (r-gridr(j))/(gridr(j+1)-gridr(j))
            Et=Ek+pot
            if (Et.lt.Ef) rejet=.false.
         end do
         stheta= dsqrt(1.d0-(2.d0*x(3)-1.d0)**2.d0)
         qp(1,i) = r*dcos(2.d0*PI*x(2))*stheta
         qp(2,i) = r*dsin(2.d0*PI*x(2))*stheta
         qp(3,i) = r*(2.d0*x(3)-1.d0)
         stheta = sqrt(1.d0-(2.d0*x(6)-1.d0)**2.d0)
         pp(1,i) = p*ee*dcos(2.d0*PI*x(5))*stheta
         pp(2,i) = p*ee*dsin(2.d0*PI*x(5))*stheta
         pp(3,i) = p*ee*(2.d0*x(6)-1.d0)
      end do
      i=109
      print*,'*************************************'
      print*,'*********pseudopart ',i,'**************'
      print*,'x,y,z',qp(1,i),qp(2,i),qp(3,i)
      r=dsqrt(qp(1,i)**2.d0+qp(2,i)**2.d0+qp(3,i)**2.d0)
      j=int(coef2*r-1.d-10)
      pot=tabpotr(j)+(tabpotr(j+1)-tabpotr(j))*
     +     (r-gridr(j))/(gridr(j+1)-gridr(j))
      print*,'r,pot',r,pot*ee
      print*,'px,py,pz',pp(1,i),pp(2,i),pp(3,i)
      p=dsqrt(pp(1,i)**2.d0+pp(2,i)**2.d0+pp(3,i)**2.d0)
      print*,'p,ek',p,0.5d0*(p**2.d0)/ee
      print*,'ektot=',pot*ee+0.5d0*(p**2.d0)/ee
      write (*,'(''Initialization of test particles completed'')')
      print*,'efficacite (%)',100.0*float(npart)/float(count)
      end           
c----------------------------------------------------------------------
      subroutine litpotexa(gridexa,potexa,nbgridexa)
c
      include 'ceq3d.f'
      real*8 potexa(0:nbtdmax),gridexa(0:nbtdmax)
      real*8 rmax,pmax,Ef
...
c
      ...
      read (1,*) rmax,pmax,Ef
      do i=0,nbgridexa
           read (1,'(3e14.6)') gridexa(i),dumx,potexa(i)
      end do	
c
      end           
c----------------------------------------------------------------------
      function ppotexa(gridexa,potexa,nbgridexa,r)
c
      include 'ceq3d.f'
      real*8 potexa(0:nbtdmax),gridexa(0:nbtdmax)
      integer nbgridexa
      real*8 ppotexa,rmax,coef,r
      integer j
c
      rmax=gridexa(nbgridexa)
      if (r.ge.rmax) then
         ppotexa=0.d0
      else
         coef=dfloat(nbgridexa)/rmax
         j=int(coef*r-1.d-10)
         ppotexa=potexa(j)+(potexa(j+1)-potexa(j))*
     +        (r-gridexa(j))/(gridexa(j+1)-gridexa(j))
      end if
      end           
c-----------------------------------------------------------
      subroutine enertot2gix(npart,nbion,nbelec,qp,nx,ny,nz,
     +     csolbig,gxbig,gybig,gzbig,nboutbig,
     +     csol,gx,gy,gz,inttab1,nbdt,pasgrid,
     +     enele,einout,elin,enejel,ekin,ekinout,
     +     enetot,nbtour,lcine,first,nbcap,liste2,titreq,rcmax,
     +     fp,dltt,potexa,gridexa,nbgridexa)
      ...
      integer nboutbig
      integer liste2(npartmax),compt
      integer nbdt,npart,nbcap,counter
      real*8 coef
      real*8 csr3s2,coefint
      real*8 xmin,xmax,ymin,ymax,zmin,zmax
      real*8 xmin2,xmax2,ymin2,ymax2,zmin2,zmax2
     ...
     +       (y.gt.ymin).and.(y.lt.ymax).and.
     +       (z.gt.zmin).and.(z.lt.zmax)) then
c            call potensg(x,nx,gx,y,ny,gy,z,nz,gz,csol,pote,
c     +           inttab1,nbdt,pasgrid)
            r=dsqrt(x**2+y**2+z**2)
            pote=ppotexa(gridexa,potexa,nbgridexa,r)
            potel=potel+coef*pote
            potepart=coef*pote
            fp(2,i)=potepart
            fp(3,i)=potepart+ekpart
            ...
            if ((potepart+ekpart).gt.0.) then
               nonlie=nonlie+1
               if (nonlie.lt.10) then
                  print*,'nonlie***************',nonlie,i
                  print*,'ekpart,potepart,etot',ekpart,potepart,etotpart
                  print*,'x,y,z',x,y,z
               end if
            end if
         else
...
c-----------------------------------------------------------
      subroutine mkpotradx(nbion,nbelec,nx,ny,nz,npart,
     +     csol,gx,gy,gz,inttab1,nbdt,pasgrid,
     +     gridrad,potrad,nbgridrad,potexa,gridexa,nbgridexa)
      include 'ceq3d.f'
      real*8 potexa(0:nbtdmax),gridexa(0:nbtdmax)
...
c     +           inttab1,nbdt,pasgrid)
      ...
      potrad=potrad/float(nbang+1)
      open (1,file='potrad.dat',status='unknown')
      do i=0,nbgridrad
         write (1,'(3e14.6)') gridrad(i),potrad(i)
     +        ,potrad(i)*dfloat(npart)/nbelec
      end do
      close (1)
      end      
